...
#         $SolFilename = Join-Path $OutputDir ('{0}_sol.txt' -f $FilenameRoot)
#         $StatsFilename = Join-Path $OutputDir ('{0}_stats.txt' -f $FilenameRoot)
#         Invoke-Expression $('{0} {1} {2} "{3}" "{4}" "{5}"' -f $Progcmd, $strategy, $param, $_.FullName, $SolFilename, $StatsFilename)
#     }
# }
function RunProg([string]$strategy, [string]$param) {
    $OutputDir = "./solved_puzzles"
    ...
    Get-ChildItem -Path './puzzles' -File | Where-Object { $_.Name -match $InitFilenameRegex } | ForEach-Object {
        ...
        Write-Host "Running: $Progcmd $strategy $param `"$($_.FullName)`" `"$SolFilename`" `"$StatsFilename`""
        Invoke-Expression $('{0} {1} {2} "{3}" "{4}" "{5}"' -f $Progcmd, $strategy, $param, $_.FullName, $SolFilename, $StatsFilename)
    }
}
